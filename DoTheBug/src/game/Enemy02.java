...
public class Enemy02 extends EnemyEntitie{
	...
	public Enemy02(Player player,float x,float y) {
		this.player=player;
		speed=-14f;
		this.x=x;
		this.y=y;
	...
	static {
		try {
			img = ImageIO.read(new File("src\\gfx\\enemy02.png"));
		} catch (IOException e) {
			try {
				img = ImageIO.read(new File("gfx\\enemy02.png"));
	...
	public void update(float timeScinceLastFrame) {
		...
		if(alive){
			size-=0.075f*timeScinceLastFrame;
